from .models import Category,Forum,Article, TranslatedArticle, Language, Question, Answer
from .scrape import ToSlug
...
class Google(Tse):
    ...
    @Tse.time_stat
    def google_api(self, query_text:str, from_language:str='auto', to_language:str='ru', **kwargs) -> Union[str,list]:
        """
...
@logger.catch
def test():
    str = 'Translators is a library which aims to bring free, multiple, enjoyable translation to individuals and students in Python. It based on the translation interface of Google, Yandex, Microsoft(Bing), Baidu, Alibaba, Tencent, NetEase(Youdao), Sogou, Deepl, etc.'
    print(google(str))

def TranslateArticle(article, trLang ):
    valid = False
    for trans in article.translatedarticle_set.all():
        if trans.language == trLang :
            valid = True
            break
    if valid == True:
        return
    for var in article.translatedarticle_set.all() :
        if var.language.langcode == 'en' :
            newTrans = TranslatedArticle(parent=article)
            newTrans.language = trLang
            newTrans.name = google(article.name,to_language=trLang.langcode)
            newTrans.slug = ToSlug(newTrans.name)
            print(newTrans)
            newTrans.save()
            for que in var.question_set.all() :
                newQue = Question(parentArticle=newTrans,content=google(que.content,to_language=trLang.langcode))
                newQue.save()
            for answer in var.answer_set.all() :
                answer = Answer(parentArticle=newTrans,content=google(answer.content,to_language=trLang.langcode))
                answer.save()
#                print(google(answer.content))
#            print(google(var.con))
#            print(var)
#    print(google(engArticle.name))
